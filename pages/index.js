//first page that runs on startup
//next js has built in router and works as /name_of_page
...
export default function Home() {
  const router = useRouter(); //inbuilt router in nextjs

  const searchInputRef = useRef(null); //hook to get input text from search input text

  //runs when button is pressed for search
  const search = (e) => {
  ...
  //tailwindcss follows mobile first design principle that is
  //design for mobile screen first then gradually for larger screen sizes
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen">
      ...
      {/*Body of home page containing search bar and buttons */}
      <form className="flex flex-col items-center mt-44 flex-grow w-4/5">
        {/*NextJS Iamge component we have to specify next js the link 
        in the config file*/}
        <Image
